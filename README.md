...
图 1.1 Saylo 桌面端场景判断与渐进学习界面，来源为 2026-08-24 仓库产品截图

</div>

> [!CAUTION]
> 这是用于人工审计的本地 README 试点预览，不代表 `saylo` 远端仓库已经更新
...
以下任务证据把首页截图、学习流程和可观察结果连接起来，让读者不需要从多张图片自行推断产品怎样工作

<div align="center">

表 2.1 Saylo 从场景判断到迁移使用的任务证据

| 阶段 | 学习者动作 | 界面证据 | 可观察结果 |
|---:|---|---|---|
| 1 判断场景 | 先判断关系、任务和回应目的 | 学习页的场景问题与候选功能 | 明确表达在当前关系中需要完成什么 |
| 2 理解来路 | 查看字面逻辑、例句、来源、传播和易混淆表达 | 理解页的分层内容卡 | 区分表达含义、传播范围和使用边界 |
| 3 主动输出 | 写出自己的短回应并接受本地证据检查 | 文字对练与课程要求 | 看到目标表达命中、长度、重复和风险边界 |
| 4 复习迁移 | 根据回忆结果进入复习，再用于文字或语音对练 | 复习调度与对练入口 | 获得下一次复习安排和新的迁移任务 |
...
flowchart TD
    A["场景判断<br/>关系与语境"] --> B["理解来路<br/>字面·来源·传播"]
    B --> C["主动输出<br/>短回应·本地检查"]
    C --> D["复习迁移<br/>AI 评析·间隔重复·对练"]
```
...
首屏主图展示场景判断；以下 3 张补充截图分别证明表达检索、来路理解和窄屏学习，没有把同一页面的装饰性变体重复列出
...
图 2.4 Saylo 移动端实际界面，截图文件为 375 × 811 像素，展示窄屏导航、场景判断和学习操作
...
以下矩阵只记录当前仓库截图和本地 README 预览能够证明的范围，缺少的产品状态保持为证据缺口

<div align="center">

表 9.1 Saylo README 视觉验收范围

| 场景 | 当前证据 | 本地预览要求 | 证据边界 |
|---|---|---|---|
| 桌面主任务 | 3 张 1428 × 919 像素的产品截图 | 首屏、检索和理解内容清晰，整页不横向溢出 | 截图覆盖亮色产品界面 |
| 窄屏主任务 | 1 张 375 × 811 像素的产品截图 | 图片完整缩放，导航和主要操作仍可辨认 | 当前材料只覆盖一条移动学习路径 |
| README 亮色 | 本地 GitHub 风格渲染 | 标题、徽章、表格、图片和 Mermaid 清晰 | 不代表所有浏览器的产品界面一致 |
| README 暗色 | 本地 GitHub 风格渲染 | 透明 SVG、文字和边界保持可读 | 当前仓库没有提供产品暗色主题截图 |
| 图片失败 | 替代文本、图题和任务证据表 | 隐藏图片后仍能理解学习任务与首次成功路径 | 无法替代像素层的界面细节 |
| 外部图片服务失败 | 徽章和产品图均来自仓库本地文件 | 核心内容不产生第三方图片请求 | 正文中的文档链接仍需要网络访问 |
