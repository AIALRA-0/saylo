...
本地优先、场景驱动的当代英语表达训练系统

[![Status](https://img.shields.io/badge/status-public_beta-E6A85C?style=flat-square)](#8-公开状态)
...
[![Node.js](https://img.shields.io/badge/Node.js-20%2B-5FA04E?style=flat-square&logo=node.js&logoColor=white)](https://nodejs.org/)

[快速开始](#4-快速开始) · [教学设计](docs/SLANG_LEARNING_SYSTEM_PLAN.md) · [内容治理](docs/CONTENT_GOVERNANCE.md) · [通用部署](docs/DEPLOYMENT.md)

</div>

## 1 产品概览

根据 `src/data/expressions.ts` 和内容审计测试，Saylo 当前收录 301 条当代英语表达，分布在 26 个语用单元
...
<div align="center">
  <img src="docs/images/saylo-library-desktop.png" alt="Saylo 表达地图桌面端界面，展示搜索、使用边界、语用单元和表达卡" width="100%">
  <p>图 1.1 Saylo 表达地图桌面端实际界面，根据 2026-08-24 部署版本截图记录</p>
</div>

## 2 学习体验

Saylo 的学习闭环把确定性证据和语言模型评析分开处理

字面命中、风险边界和课程要求由本地规则核对

语法、搭配、自然度和文化分寸交给可选的云端模型分析
...
<div align="center">
  <img src="docs/images/saylo-learning-desktop.png" alt="Saylo 桌面端学习界面，展示场景判断、表达边界和渐进学习步骤" width="100%">
  <p>图 2.2 桌面端场景判断与渐进学习，根据 2026-08-24 部署版本截图记录</p>
</div>

<div align="center">
  <img src="docs/images/saylo-understanding-desktop.png" alt="Saylo 理解页桌面端界面，展示字面逻辑、真实例句、来源、传播和易混淆表达" width="100%">
  <p>图 2.3 理解来路阶段实际界面，根据 2026-08-24 部署版本截图记录</p>
</div>

<div align="center">
  <img src="docs/images/saylo-learning-mobile.png" alt="Saylo 移动端学习界面，展示响应式导航、场景判断和表达学习卡" width="390">
  <p>图 2.4 移动端实际界面，根据浏览器验收记录，截图视口宽度为 390 像素</p>
</div>
...
<p>表 3.1 Saylo 当前公开测试版能力，来源：当前仓库数据、页面和自动化测试</p>
...
| 学习数据 | 当前浏览器保存进度、活动、收藏和反馈；支持 JSON 备份、恢复与清空 |
| 使用终端 | 桌面与移动端响应式布局，可作为渐进式网页应用添加到主屏幕 |
...
- 支持现代 JavaScript、Web Speech 和 WebRTC 的浏览器；具体语音能力取决于浏览器实现
...
<p>表 5.1 评析证据的职责边界，来源：当前仓库评析实现和测试</p>
...
图 6.1 Saylo 应用、身份代理和可选模型供应商之间的通用数据流，根据仓库公开接口绘制
...
- 生产部署可以通过任意兼容前向认证的反向代理保护，应用服务建议只监听服务器回环地址
...
内容库已经完成结构、重复、风险字段、中性替代、参考答案和日常功能覆盖检查

表达来源说明仍需要真实社群语言顾问和美国英语教师完成外部审校，因此仓库内容不代表教师或来源社群认证

仓库当前没有附带开源许可证

公开可见只代表源码可以被阅读，不自动授予复制、修改、再分发或商业使用权
...
src/lib/        # FSRS、教练、语音、统计和内容策略
src/pages/      # 学习者直接操作的页面
server/         # 文字评析、个人密钥和实时语音接口
deploy/         # 通用反向代理和系统服务配置示例
docs/           # 教学方案、内容治理、通用部署手册、审计记录和产品图
```

## 11 生产部署

推荐链路为域名与传输层安全终止 → 反向代理 → 可选身份代理 → Saylo 回环服务

仓库提供 Nginx 网页服务器和 systemd 系统服务的通用配置示例，域名、账户、目录和身份头均使用占位值

[通用部署手册](docs/DEPLOYMENT.md) 说明环境变量、身份代理契约、密钥隔离和发布后健康检查
...
[1] OpenAI, “Text generation,” *OpenAI API Documentation*. [在线]. 可用: https://developers.openai.com/api/docs/guides/text

[2] OpenAI, “Realtime API with WebRTC,” *OpenAI API Documentation*. [在线]. 可用: https://developers.openai.com/api/docs/guides/realtime-webrtc

[3] DeepSeek, “JSON Output,” *DeepSeek API Docs*. [在线]. 可用: https://api-docs.deepseek.com/guides/json_mode
